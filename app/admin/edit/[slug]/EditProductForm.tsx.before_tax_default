...
export default function EditProductForm({ initialProduct, categories }: EditProductFormProps) {
    ...
    const handleFieldChange = (e: React.ChangeEvent<HTMLInputElement | HTMLTextAreaElement | HTMLSelectElement>) => {
        ...
        setFormData(prev => {
            ...
            if (['price', 'discount', 'stock', 'year', 'taxRate'].includes(name)) {
    ...
    const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>, type: 'main' | 'secondary' | 'video') => {
        ...
        switch (type) {
            case 'main': setNewMainImage(file); setMainImagePreview(URL.createObjectURL(file)); break;
            case 'secondary': setNewSecondaryImage(file); setSecondaryImagePreview(URL.createObjectURL(file)); break;
            case 'video': setNewVideo(file); setVideoFileName(file.name); break;
        }
    };

    const handleDeleteFile = async (type: 'secondary' | 'video') => {
        if (!confirm(`هل أنت متأكد أنك تريد حذف هذا ${type === 'video' ? 'الفيديو' : 'الصورة'}؟`)) return;
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            if (!token) throw new Error("لم يتم العثور على توكن المصادقة. الرجاء تسجيل الدخول مرة أخرى.");

            let updatedData = { ...formData };
            const newSlug = generateSlug(formData.name || initialProduct.name);

            if (newMainImage) updatedData.imageUrl = await uploadFile(newMainImage, `products/${newSlug}/main-${newMainImage.name}`, setMainImgUpProgress);
            if (newSecondaryImage) updatedData.secondaryImageUrl = await uploadFile(newSecondaryImage, `products/${newSlug}/secondary-${newSecondaryImage.name}`, setSecondaryImgUpProgress);
            if (newVideo) updatedData.videoUrl = await uploadFile(newVideo, `products/${newSlug}/video-${newVideo.name}`, setVideoUpProgress);

            updatedData.updatedAt = new Date().toISOString();
            updatedData.slug = newSlug;

            const response = await fetch(`/api/products/${encodeURIComponent(originalSlug)}`, {
                method: 'PUT',
            ...
            console.log('✅ تم تحديث المنتج بنجاح. تحديث الكاش يتم تلقائيًا على الخادم.');
            alert('تم تحديث المنتج بنجاح! سيظهر التحديث على الموقع فورًا.');
                ...
                <form onSubmit={handleSubmit} className="space-y-6">
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div>
                            ...
                            <input type="text" id="sku" name="sku" value={formData.sku || ''} className="mt-1 block w-full border border-gray-300 rounded-md shadow-sm py-2 px-3 bg-gray-100" readOnly />
                        </div>
                        <div className="md:col-span-2">
                        ...
                        <div>
                           ...
                           <label htmlFor="taxRate" className="block text-sm font-medium text-gray-700 mt-3">🆕 نسبة الضريبة (%)</label>
                           <input type="number" step="1" id="taxRate" name="taxRate" value={formData.taxRate ?? 14} onChange={handleFieldChange} placeholder="مثال: 14" min="0" max="100" className="mt-1 block w-full border border-gray-300 rounded-md shadow-sm py-2 px-3" />
