...
// تعريف أنواع البيانات الصارمة للامتثال البرمجي
...
// دالة تحويل الحالات المالية والتنظيمية إلى اللغة العربية مع الكلاسات الخاصة بها
...
const OrdersClient = ({ initialOrders }: OrdersClientProps) => {
    // تهيئة حالة الطلبات حياً بالـ State المعزول
    const [orders, setOrders] = useState<Order[]>(initialOrders);

    const handleStatusChange = async (orderId: string, newStatus: OrderStatus) => {
        // تحديث تفاؤلي سريع للواجهة أمام الموظف (Optimistic Update)
        ...
        try {
            ...
            if (!response.ok) {
                // تراجع تلقائي عن التحديث الفوري في حال حدوث فشل بالشبكة لحماية الجرد
    ...
    // دالة تصدير كشف الطلبات المحدث بالإكسيل لوردية الكاشير والمندوبين
    const exportToExcel = () => {
    ...
    // 🎯 الميزة الذكية الخارقة: بتر الصفر المحلي وضخ مفتاح مصر (2) قسرياً لرابط الواتساب
                                    ...
                                    {order.items.map((item, index) => (
                                        <li key={index} className={styles.item}>
                                            ...
                                            <div className={styles.itemDetails}>
                                                {/* 🎯 تم تصحيح العرض وإلغاء الرموز المكسورة */}
