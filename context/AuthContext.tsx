...
interface AuthContextType {
  user: User | null;
  isAdmin: boolean;
  isEditor: boolean; // 🎯 حقن الصلاحية لإنقاذ شاشات الموظفين الجدد
  loading: boolean;
  logout: () => Promise<void>;
}
...
export const AuthProvider = ({ children }: { children: ReactNode }) => {
  ...
  const [isEditor, setIsEditor] = useState(false); // 🎯 تتبع صلاحيات محررين مخزن الـ POS
        ...
        if (currentUser) {
          ...
          try {
            ...
            if (response.ok) {
                ...
                // 👑 المالك والآدمن الأعلى للمنظومة والبنك
                const ADMIN_EMAIL = 'waelwasel37@gmail.com';
                const userIsAdmin = userEmail === ADMIN_EMAIL;
                setIsAdmin(userIsAdmin);

                // 🛡️ القائمة الثلاثية الموحدة والمحمية جوه السيرفر للموظفين المعتمدين
                ...
                // 🎯 تحويل انسيابي ومستقر فور التحقق الناجح لمنع الوميض والتكرار
